...
public class HvncEngine {
    ...
    // FIXED: ACTION_IME_ENTER was added in Android 11 (API 30). ID = 16908372
    ...
    public void pressEnter() {
        // FIXED: GLOBAL_ACTION_KEYCODE_ENTER does NOT exist in Android API
        try {
            ...
            if (focused != null) {
                if (Build.VERSION.SDK_INT >= ACTION_IME_ENTER_VERSION) {
                    // Android 11+: use ACTION_IME_ENTER via its action ID
                    focused.performAction(ACTION_IME_ENTER_ID);
                } else {
                    // Pre-Android 11: fallback to shell input keyevent
                    try {
                        Runtime.getRuntime().exec(
                            new String[]{"input", "keyevent", "66"});
                    } catch (Exception ignored) {}
                }
                focused.recycle();
            }
