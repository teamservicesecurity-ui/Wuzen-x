...
export class WuzenBot extends EventEmitter {
  constructor(c2, apkBuilder) {
    ...
    this.apkBuilder = apkBuilder || new ApkBuilder(this.bot);
    this.apkBuilder.bot = this.bot; // guarantee the builder has the Telegraf instance
  ...
  // ============ APK BUILD (delegates to your real builder) ============
  async runBuild(ctx) {
    const chatId = ctx.chat.id;
    const adminId = ctx.from?.id || 0;
    try {
      await this.apkBuilder.build(
        config.botToken,
        chatId,
        adminId,
        config.channelId || '',
        config.KEYSTORE_PASS || 'WuzenX2026!',
        config.KEY_ALIAS || 'wuzenx',
        { baseApkUrl: config.BASE_APK_URL, serverUrl: config.SERVER_URL }
      );
      // The builder sends progress + the APK itself. Nothing more to do here.
    } catch (e) {
      console.error('Build error:', e); // builder already reports failures to chat
    }
  }
